 ...
 * (Accueil/Catégorie/Ventes/Profil) et le panier flottant vivent dans
 * (tabs)/layout.tsx, pas ici, pour qu'un détail produit (produits/[id])
 * puisse s'en affranchir tout en gardant ce même cadre "carte centrée".
 */
  ...
  return (
    <RouteGuard>
      {/* h-dvh + overflow-hidden (pas juste min-h-dvh comme le shell principal) :
          le panier/"Mode client" flottants de (tabs)/layout.tsx sont ancrés en
          absolute — sans ce plafond, body (min-h-full, voir app/layout.tsx)
          grandit avec le contenu et fait défiler toute la page au lieu du seul
          conteneur interne, cassant cet ancrage. */}
